...
class JantungKoronerScoringService
{
    /**
    ...
    {
        $options = config('jantung_koroner_skrining.yes_no_options');
        $prefix = config('jantung_koroner_skrining.question_prefix');
        ...
        }, config('jantung_koroner_skrining.items'));
    }

    public function maxScore(): int
    {
        return (int) array_sum(array_column(config('jantung_koroner_skrining.items'), 'score_ya'));
    }

    public function hasilKategori(int $total): string
    {
        $tinggiMin = (int) config('jantung_koroner_skrining.tinggi_min', 11);
        $sedangMin = (int) config('jantung_koroner_skrining.sedang_min', 6);
    ...
    public function calculateTotal(array $answers): int
    {
        ...
        foreach (config('jantung_koroner_skrining.items') as $item) {
    ...
    public function buildSummary(array $answers): string
    {
        ...
        $lines = [
            '📋 Hasil Skrining Jantung Koroner',
            '',
