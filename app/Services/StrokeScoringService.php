...
class StrokeScoringService
{
    ...
    {
        $options = config('stroke_skrining.yes_no_options');
        $prefix = config('stroke_skrining.question_prefix');
        ...
        }, config('stroke_skrining.items'));
    }

    public function maxScore(): int
    {
        return (int) array_sum(array_column(config('stroke_skrining.items'), 'score_ya'));
    }

    public function hasilKategori(int $total): string
    {
        $tinggiMin = (int) config('stroke_skrining.tinggi_min', 11);
        $sedangMin = (int) config('stroke_skrining.sedang_min', 6);
    ...
    public function calculateTotal(array $answers): int
    {
        ...
        foreach (config('stroke_skrining.items') as $item) {
