...
class PenyakitGinjalScoringService
{
    /**
    ...
    {
        $options = config('penyakit_ginjal_skrining.yes_no_options');
        $prefix = config('penyakit_ginjal_skrining.question_prefix');
        ...
        }, config('penyakit_ginjal_skrining.items'));
    }

    public function maxScore(): int
    {
        return (int) array_sum(array_column(config('penyakit_ginjal_skrining.items'), 'score_ya'));
    }

    public function hasilKategori(int $total): string
    {
        $tinggiMin = (int) config('penyakit_ginjal_skrining.tinggi_min', 11);
        $sedangMin = (int) config('penyakit_ginjal_skrining.sedang_min', 6);
    ...
    public function calculateTotal(array $answers): int
    {
        ...
        foreach (config('penyakit_ginjal_skrining.items') as $item) {
    ...
    public function buildSummary(array $answers): string
    {
        ...
        $lines = [
            '📋 Hasil Skrining Penyakit Ginjal',
            '',
