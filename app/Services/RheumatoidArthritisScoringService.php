...
class RheumatoidArthritisScoringService
{
    /**
    ...
    {
        $options = config('rheumatoid_arthritis_skrining.yes_no_options');
        $prefix = config('rheumatoid_arthritis_skrining.question_prefix');
        ...
        }, config('rheumatoid_arthritis_skrining.items'));
    }

    public function maxScore(): int
    {
        return (int) array_sum(array_column(config('rheumatoid_arthritis_skrining.items'), 'score_ya'));
    }

    public function hasilKategori(int $total): string
    {
        $tinggiMin = (int) config('rheumatoid_arthritis_skrining.tinggi_min', 10);
        $sedangMin = (int) config('rheumatoid_arthritis_skrining.sedang_min', 5);
    ...
    public function calculateTotal(array $answers): int
    {
        ...
        foreach (config('rheumatoid_arthritis_skrining.items') as $item) {
    ...
    public function buildSummary(array $answers): string
    {
        ...
        $lines = [
            '📋 Hasil Skrining Rheumatoid Arthritis (RA)',
            '',
