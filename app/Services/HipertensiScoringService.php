...
class HipertensiScoringService
{
    /**
    ...
    {
        $options = config('hipertensi_skrining.yes_no_options');
        $prefix = config('hipertensi_skrining.question_prefix');
        ...
        }, config('hipertensi_skrining.items'));
    }

    public function maxScore(): int
    {
        return (int) array_sum(array_column(config('hipertensi_skrining.items'), 'score_ya'));
    }

    public function hasilKategori(int $total): string
    {
        $tinggiMin = (int) config('hipertensi_skrining.tinggi_min', 11);
        $sedangMin = (int) config('hipertensi_skrining.sedang_min', 6);
    ...
    public function calculateTotal(array $answers): int
    {
        ...
        foreach (config('hipertensi_skrining.items') as $item) {
